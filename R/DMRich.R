...
DMRichCpG <- function(sigRegions = sigRegions,
  ...
  stopifnot(genome %in% c("hg38", "hg19", "mm10", "mm9", "rheMac10", "rheMac8", "rn6", "danRer11", "galGal6", "bosTau9", "panTro6", "dm6", "susScr11", "canFam3", "Tthymallus", "ThyArc1.0"))
  ...
  lapply(CGterms, function(term){
    ...
    CGannotationsFiltered <- CGannotations %>%
      dplyr::filter(stringr::str_detect(type, term))
    
    sigRegionsOverlap <- sigRegions %>% 
      dplyr::mutate(n_overlaps = plyranges::count_overlaps(.,CGannotationsFiltered)) %>%
      dplyr::filter(n_overlaps > 0)
